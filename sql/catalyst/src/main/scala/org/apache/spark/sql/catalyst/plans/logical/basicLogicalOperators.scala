...
case class Join(
    ...
    notNullAttributes: Option[Set[ExprId]] = None)
  extends BinaryNode with PredicateHelper {

  private def updateNullabilityOf(attrs: Seq[Attribute]): Seq[Attribute] =
    notNullAttributes.map { notNullAttrs =>
      attrs.map {
        case a if a.nullable && notNullAttrs.contains(a.exprId) =>
