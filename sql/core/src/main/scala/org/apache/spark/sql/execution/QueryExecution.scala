...
class QueryExecution(
  ...
  private def printExtendedString[T <: QueryPlan[T]](
      plan: QueryPlan[T],
      append: String => Unit,
      formatted: Boolean,
      verbose: Boolean,
  ...
  private def writePlans(formatted: Boolean, append: String => Unit, maxFields: Int): Unit = {
    ...
    try {
      ...
      printExtendedString(analyzed, append, formatted, verbose, addSuffix, maxFields)
      append("\n== Optimized Logical Plan ==\n")
      printExtendedString(optimizedPlan, append, formatted, verbose, addSuffix, maxFields)
      append("\n== Physical Plan ==\n")
      printExtendedString(executedPlan, append, formatted, verbose, addSuffix, maxFields)
    } catch {
      case e: AnalysisException => append(e.toString)
    }
  }
