...
-- to outside an IN
--
select udf(udf(count(*))) from global_temp.tenk1 a where udf(udf(unique1)) in
  (select udf(unique1) from global_temp.tenk1 b join global_temp.tenk1 c using (unique1)
...
--
select udf(count(*)) from global_temp.tenk1 x where
...
select udf(udf(count(*))) from global_temp.tenk1 x where
  udf(x.unique1) in (select udf(a.f1) from int4_tbl a,float8_tbl b where udf(udf(a.f1))=b.f1) and
  udf(x.unique1) = 0 and
...
from
  (select udf(t3.tenthous) as x1, udf(coalesce(udf(t1.stringu1), udf(t2.stringu1))) as x2
   ...
   left join global_temp.tenk1 t2 on udf(t1.unique1) = udf(t2.unique1)
   join global_temp.tenk1 t3 on t1.unique2 = udf(t3.unique2)) ss,
...
select udf(count(*)) from
  (select * from global_temp.tenk1 x order by udf(x.thousand), udf(udf(x.twothousand)), x.fivethous) x
  left join
  (select * from global_temp.tenk1 y order by udf(y.unique2)) y
...
select udf(count(*)) from global_temp.tenk1 a, global_temp.tenk1 b
...
from global_temp.tenk1 a left join global_temp.tenk1 b on a.unique2 = udf(b.tenthous)
...
SELECT udf(qq), udf(udf(unique1))
  ...
  USING (qq)
  INNER JOIN global_temp.tenk1 c ON udf(qq) = udf(unique2);
...
select udf(count(*)) from
  global_temp.tenk1 a join global_temp.tenk1 b on udf(a.unique1) = udf(b.unique2)
  left join global_temp.tenk1 c on udf(a.unique2) = udf(b.unique1) and udf(c.thousand) = udf(udf(a.thousand))
...
select udf(b.unique1) from
  global_temp.tenk1 a join global_temp.tenk1 b on udf(a.unique1) = udf(b.unique2)
  left join global_temp.tenk1 c on udf(b.unique1) = 42 and c.thousand = udf(a.thousand)
...
(
  ...
  from int8_tbl left join global_temp.tenk1 on (udf(q2) = udf(unique2))
) ss
...
select udf(q1), udf(unique2), udf(thousand), udf(hundred)
  from int8_tbl a left join global_temp.tenk1 b on udf(q1) = udf(unique2)
...
select udf(f1), udf(unique2), case when udf(udf(unique2)) is null then udf(f1) else 0 end
  from int4_tbl a left join global_temp.tenk1 b on udf(f1) = udf(udf(unique2))
...
select udf(a.unique1), udf(b.unique1), udf(c.unique1), udf(coalesce(b.twothousand, a.twothousand))
  from global_temp.tenk1 a left join global_temp.tenk1 b on udf(b.thousand) = a.unique1                       left join global_temp.tenk1 c on udf(c.unique2) = udf(coalesce(b.twothousand, a.twothousand))
...
select * from
  ...
  left join
    (global_temp.tenk1 as a1 full join (select udf(1) as id) as yy on (udf(a1.unique1) = udf(yy.id)))
...
select udf(t1.uunique1) from
  global_temp.tenk1 t1 join tenk2 t2 on t1.two = udf(t2.two); -- error, prefer "t1" suggestion
select udf(udf(t2.uunique1)) from
  global_temp.tenk1 t1 join tenk2 t2 on udf(t1.two) = t2.two; -- error, prefer "t2" suggestion
select udf(uunique1) from
  global_temp.tenk1 t1 join tenk2 t2 on udf(t1.two) = udf(t2.two); -- error, suggest both at once
