...
import org.apache.spark.sql.catalyst.analysis.{CastSupport, ResolveUnion}
...
object PropagateEmptyRelation extends Rule[LogicalPlan] with PredicateHelper with CastSupport {
  ...
  private def nullValueProjectList(plan: LogicalPlan): Seq[NamedExpression] =
    plan.output.map{ a => Alias(cast(Literal(null), a.dataType), a.name)() }
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transformUpWithNewOutput {
    case p: Union if p.children.exists(isEmptyLocalRelation) =>
      ...
      if (newChildren.isEmpty) {
        ...
        if (newChildren.size > 1) {
          p.copy(children = newChildren) -> Nil
        } else {
          val newPlan = newChildren.head
          val outputAliases = p.output.zip(newPlan.output).map { case (oldAttr, newAttr) =>
            // Since `Union` merges the input data types to compute output types, their types might
            // be different between each other. So, we need a cast here if they are mismatched.
            ...
            Alias(newExpr, oldAttr.name)(explicitMetadata = newExplicitMetadata)
          }
          val proj = Project(outputAliases, newPlan)
          val attrMapping = p.output.zip(proj.output)
          proj -> attrMapping
        }
      }
    ...
    case p: UnaryNode if p.children.nonEmpty && p.children.forall(isEmptyLocalRelation) =>
      (p match {
        ...
        // An aggregate with non-empty group expression will return one output row per group when
        // the input to the aggregate is not empty. If the input to the aggregate is empty then all
        // groups will be empty and thus the output will be empty. If we're working on batch data,
        // we can then treat the aggregate as redundant.
        //
