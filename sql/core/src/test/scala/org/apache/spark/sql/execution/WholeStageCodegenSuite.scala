...
import org.apache.spark.sql.catalyst.expressions.codegen.{CodegenContext, CodeAndComment, CodeGenerator}
...
class WholeStageCodegenSuite extends SparkPlanTest with SharedSQLContext {
  ...
  test("SPARK-XXXXX turn off whole-stage codegen if bytecode size goes over HugeMethodLimit") {
    withSQLConf(SQLConf.WHOLESTAGE_MAX_LINES_PER_FUNCTION.key -> Int.MaxValue.toString) {
