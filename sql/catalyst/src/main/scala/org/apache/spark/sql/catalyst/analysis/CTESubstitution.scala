...
import org.apache.spark.sql.catalyst.plans.logical.{Distinct, LogicalPlan, RecursiveUnion, SubqueryAlias, With, WithRecursive}
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    cteSubstitutedPlan.resolveOperatorsUp {
      case WithRecursive(child, cteName, cteColumnNames, initPlan, recursivePlan, distinct) =>
        ...
        val recursiveUnion = if (distinct) {
          Distinct(RecursiveUnion(initPlan, newRecursivePlan))
        } else {
          RecursiveUnion(initPlan, newRecursivePlan)
        }
