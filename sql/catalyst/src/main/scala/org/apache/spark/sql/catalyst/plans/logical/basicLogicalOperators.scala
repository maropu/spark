...
case class RecursiveState(output: Seq[Attribute]) extends LeafNode {

  override def computeStats(): Statistics = Statistics(
    sizeInBytes = conf.defaultSizeInBytes
  )
...
case class RecursiveReferences(output: Seq[Attribute], initPlanStats: Statistics) extends LeafNode {

  override def computeStats(): Statistics = Statistics(
    sizeInBytes = initPlanStats.sizeInBytes,
    rowCount = initPlanStats.rowCount
  )
